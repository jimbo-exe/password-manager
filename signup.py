...
def showWindow():
	...
	instruction = Label(root, text='Please Enter Master Password',  font="Helvetica 20 bold") # This puts a label, so just a piece of text saying 'please enter blah5
	instruction.grid(row=0, column=0, columnspan=3, sticky=E) # This just puts it in the window, on row 0, col 0. If you want to learn more look up a tkinter tutorial :)

	headingL = Label(root, text='\nWelcome to the Password Keeper. We need you to enter a global password which will be asked each time you want to view stored passwords.\n',justify='left',wraplength=300) # This just does the same as above, instead with the text new username.
	...
	pwdE = Entry(root, show='*') # Same as above, yet 'show="*"' What this does is replace the text with *, like a password box :D
	pwdE.grid(row=5, column=1) # ^^

	confirmE = Entry(root, show='*') # Same as above, yet 'show="*"' What this does is replace the text with *, like a password box :D
	confirmE.grid(row=6, column=1) # ^^

	signupB = Button(root, text='Sign Up', fg="green", command = checkPwd)#, command=storeGlobalPwd) # This creates the button with the text 'signup', when you click it, the command 'fssignup' will run. which is the def
